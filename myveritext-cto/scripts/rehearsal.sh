...
echo "== 1) Matters =="
MATTERS_RESP=$(curl -s "$BASE_URL/api/matters" "${header[@]}")
if ! echo "$MATTERS_RESP" | jq -e . >/dev/null 2>&1; then
echo "Non-JSON response from /api/matters. If Cloud Run auth is enabled, set ID_TOKEN:"
echo ' export ID_TOKEN="$(gcloud auth print-identity-token)"'
echo "$MATTERS_RESP"
exit 1
fi
echo "$MATTERS_RESP" | jq '.data[0] | {id, referenceNumber, title}'
